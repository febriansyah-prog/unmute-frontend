    ...
    <div className="min-h-screen pb-20 dark:bg-black transition-colors duration-500">
      ...
      <main className="max-w-7xl mx-auto px-6 animate-fade-in space-y-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h2 className="text-4xl font-black text-brand-purple-dark dark:text-white uppercase tracking-tight">Manajemen <span className="text-brand-purple">Perwakilan</span></h2>
            <p className="text-gray-500 dark:text-gray-400 font-medium">Kelola siswa delegasi dari sekolah yang telah dikunjungi untuk Bootcamp.</p>
          </div>
          <a
        ...
        {loading ? (
          ...
          <div className="glass-card dark:bg-gray-900/50 rounded-[2.5rem] overflow-hidden premium-shadow">
            <div className="p-8 sm:p-10 border-b border-gray-100 dark:border-gray-800">
              <h3 className="text-2xl font-black text-brand-purple-dark dark:text-white uppercase">Sekolah <span className="text-brand-purple">Telah Dikunjungi</span></h3>
            </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                  {visitedSchools.length > 0 ? (
                        ...
                        <td className="px-10 py-6">
                          <div className="flex justify-center gap-3">
                            ...
                            <button 
                              ...
                              className="px-4 py-2 rounded-xl bg-brand-lime/20 text-brand-purple-dark dark:text-white font-bold text-xs uppercase tracking-wider hover:bg-brand-lime transition-all flex items-center gap-2"
                            >
                    ...
                    <tr><td colSpan={3} className="px-10 py-20 text-center text-gray-400 dark:text-gray-500 italic">Belum ada sekolah yang ditandai Telah Dikunjungi.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {isAddModalOpen && selectedSchool && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-brand-purple-dark/40 backdrop-blur-sm p-6 animate-fade-in">
          <div className="w-full max-w-md glass-card dark:bg-gray-900/50 rounded-[2rem] p-8 sm:p-10 premium-shadow border-white/50">
            <h2 className="text-2xl font-black text-brand-purple-dark dark:text-white mb-1">{isEditing ? "Edit Siswa" : "Tambah Siswa"}</h2>
            ...
            <form onSubmit={handleSaveDelegate} className="space-y-5">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 px-1">NISN</label>
                <input
                  type="text"
                  value={nisn}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 px-1">Nama Lengkap</label>
                <input
                  type="text"
                  value={name}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 px-1">Topik Materi</label>
                <select
                  value={topic}
                  onChange={(e) => setTopic(e.target.value)}
                  className="w-full bg-gray-50 dark:bg-gray-800 dark:text-white border-2 border-transparent focus:border-brand-purple focus:bg-white rounded-2xl p-4 outline-none transition-all font-medium cursor-pointer"
                >
      ...
      {isListModalOpen && selectedSchool && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-brand-purple-dark/40 backdrop-blur-sm p-4 sm:p-6 animate-fade-in">
          <div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto glass-card dark:bg-gray-900/50 rounded-[2.5rem] p-6 sm:p-10 premium-shadow border-white/50">
            <div className="flex justify-between items-start mb-8">
              <div>
                <h2 className="text-2xl font-black text-brand-purple-dark dark:text-white mb-1">Daftar Perwakilan</h2>
                <p className="text-sm text-brand-purple font-bold opacity-60 uppercase tracking-widest">{selectedSchool.name}</p>
              </div>
              <button onClick={() => setIsListModalOpen(false)} className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-200 font-bold">✕</button>
            </div>
            ...
            <div className="mb-6 flex flex-col sm:flex-row sm:justify-between items-start sm:items-center bg-brand-purple/5 p-4 rounded-2xl border border-brand-purple/10 gap-3">
              <span className="font-bold text-brand-purple-dark dark:text-white">Total: {delegates.length}/10 Siswa</span>
            ...
            {loadingDelegates ? (
              ...
              <div className="overflow-x-auto rounded-2xl border border-gray-100 dark:border-gray-800">
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-50 text-gray-500 dark:text-gray-400 text-[10px] font-black uppercase tracking-widest">
                    ...
                    {delegates.length > 0 ? (
                      delegates.map((d, idx) => (
                        <tr key={d.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 text-gray-400 dark:text-gray-500 font-bold">{idx + 1}</td>
                          <td className="px-6 py-4 font-mono text-sm text-gray-600 dark:text-gray-400">{d.nisn}</td>
                          <td className="px-6 py-4 font-bold text-gray-800 dark:text-gray-200">{d.name}</td>
                          <td className="px-6 py-4">
                            <span className="px-3 py-1 bg-brand-lime/10 text-brand-purple-dark dark:text-white rounded-full text-[10px] font-black border border-brand-lime/20">{d.topic}</span>
                          </td>
                          <td className="px-6 py-4">
                      ...
                      <tr><td colSpan={5} className="px-6 py-10 text-center text-gray-400 dark:text-gray-500 italic">Belum ada siswa yang ditambahkan.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
