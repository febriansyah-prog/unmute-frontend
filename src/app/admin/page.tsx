    ...
    <div className="min-h-screen pb-20 dark:bg-black transition-colors duration-500">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-brand-lime/20 text-brand-purple-dark dark:text-white rounded-xl text-sm font-black uppercase tracking-wider hover:bg-brand-lime hover:text-brand-purple-dark dark:text-white transition-all"
            >
              Perwakilan
            </button>
            <button
              onClick={() => setIsPasswordModalOpen(true)}
              className="px-4 py-2 bg-gray-100 text-gray-600 dark:text-gray-400 rounded-xl text-sm font-black uppercase tracking-wider hover:bg-gray-200 transition-all"
            >
              Ganti Sandi
            </button>
            <button
        ...
        {loading ? (
          ...
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { label: "Total Sekolah", val: totalSchools, color: "text-gray-800 dark:text-gray-200", bg: "bg-white" },
                ...
                <div key={i} className={`${stat.bg} p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm transition-transform hover:scale-105 duration-300`}>
            ...
            <div className="glass-card dark:bg-gray-900/50 rounded-[2.5rem] overflow-hidden premium-shadow">
              <div className="p-6 sm:p-10 flex flex-col md:flex-row justify-between gap-6 border-b border-gray-100 dark:border-gray-800">
                <h2 className="text-2xl font-black text-brand-purple-dark dark:text-white uppercase">Manajemen <span className="text-brand-purple">Booking</span></h2>
                
                <div className="flex flex-col sm:flex-row gap-4">
                  <a
                    href={`${API_URL}/api/export/bookings`}
                    className="flex items-center justify-center gap-2 px-6 py-3 bg-brand-lime text-brand-purple-dark dark:text-white rounded-2xl font-black text-sm uppercase tracking-wider hover:bg-brand-lime-hover transition-all premium-shadow"
                  >
                    📥 Excel
                  </a>
                    ...
                    {filteredBookings.length > 0 ? (
                      filteredBookings.map((b, index) => (
                        <tr key={b.id} className="glass-row-hover dark:hover:bg-gray-800 transition-colors group">
                          <td className="px-8 py-6 text-gray-300 dark:text-gray-600 dark:text-gray-600 dark:text-gray-400 font-bold text-center">{index + 1}</td>
                          <td className="px-8 py-6 font-black text-gray-800 dark:text-gray-200 dark:text-gray-200 group-hover:text-brand-purple dark:group-hover:text-brand-lime transition-colors">{b.school_name}</td>
                          <td className="px-8 py-6 text-gray-600 dark:text-gray-400 dark:text-gray-400 font-medium">{b.pic}</td>
                          <td className="px-8 py-6 text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-400 font-mono text-xs">{b.phone}</td>
                          ...
                          <td className="px-8 py-6">
                            <div className="flex justify-center gap-2">
                              ...
                              <button onClick={() => handleEditBooking(b)} title="Edit Booking" className="w-10 h-10 rounded-xl bg-brand-lime/20 text-brand-purple-dark dark:text-white hover:bg-brand-lime transition-all flex items-center justify-center">✏️</button>
                      ...
                      <tr><td colSpan={6} className="px-8 py-20 text-center text-gray-400 dark:text-gray-500 italic">Belum ada data booking.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {editingBooking && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-brand-purple-dark/40 backdrop-blur-sm p-6 animate-fade-in">
          <div className="w-full max-w-md glass-card dark:bg-gray-900/50 rounded-[2rem] p-8 sm:p-10 premium-shadow border-white/50">
            <h2 className="text-2xl font-black text-brand-purple-dark dark:text-white mb-1">Edit Booking</h2>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 px-1">Nama PIC</label>
                <input
                  type="text"
                  value={editPic}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 px-1">No WhatsApp</label>
                <input
                  type="text"
                  value={editPhone}
                  onChange={(e) => setEditPhone(e.target.value.replace(/\D/g, ""))}
                  className="w-full bg-gray-50 dark:bg-gray-800 dark:text-white border-2 border-transparent focus:border-brand-purple focus:bg-white rounded-2xl p-4 outline-none transition-all font-mono"
                />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-gray-400 dark:text-gray-500 px-1">Tanggal Kunjungan</label>
                <input
                  type="date"
                  value={editDate}
                  min="2026-05-05"
                  ...
                  className="w-full bg-gray-50 dark:bg-gray-800 dark:text-white border-2 border-transparent focus:border-brand-purple focus:bg-white rounded-2xl p-4 outline-none transition-all"
                />
              </div>
            </div>
      ...
      {isPasswordModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-3xl p-8 shadow-2xl relative border border-white/20">
            <button
              onClick={() => { setIsPasswordModalOpen(false); setPasswordError(""); setOldPassword(""); setNewPassword(""); }}
              className="absolute top-4 right-4 text-gray-400 dark:text-gray-500 hover:text-red-500 bg-gray-100 hover:bg-red-50 rounded-full w-8 h-8 flex items-center justify-center transition-colors"
            >
              ✕
            </button>
            <h2 className="text-2xl font-black text-brand-purple-dark dark:text-white uppercase tracking-tight mb-2">Ganti Sandi</h2>
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-6">Pastikan sandi baru Anda aman dan mudah diingat.</p>
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 px-1">Sandi Lama</label>
                <input
                  type="password"
                  className="w-full bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl border-2 border-gray-100 dark:border-gray-800 p-3 outline-none focus:border-brand-purple focus:bg-white transition-all"
                  value={oldPassword}
              ...
              <div className="space-y-2">
                <label className="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500 px-1">Sandi Baru (Min. 6 Karakter)</label>
                <input
                  type="password"
                  className="w-full bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl border-2 border-gray-100 dark:border-gray-800 p-3 outline-none focus:border-brand-purple focus:bg-white transition-all"
                  value={newPassword}
              ...
              <div className="pt-4 flex justify-end gap-3">
                <button
                  ...
                  className="px-6 py-3 rounded-xl font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-100 transition-colors uppercase tracking-wider text-sm"
                >
                  Batal
                </button>
                <button
